...
function Starfield() {
  ...
  React.useEffect(() => {
    ...
    const init = () => {
      const layers = [
        { n: (W * H) / 4200, sp: 0.05, r: [0.4, 1.0] },
        { n: (W * H) / 6500, sp: 0.12, r: [0.7, 1.5] },
        { n: (W * H) / 11000, sp: 0.22, r: [1.1, 2.2] }
      ];
      field = [];
      layers.forEach((L) => {
        for (let i = 0; i < Math.floor(L.n); i++) {
          field.push({ x: Math.random() * W, y: Math.random() * H, r: L.r[0] + Math.random() * (L.r[1] - L.r[0]), sp: L.sp, tw: Math.random() * Math.PI * 2, tws: Math.random() * 0.03 + 0.006 });
        }
      });
    ...
    const frame = () => {
      ...
      for (let i = 0; i < meteors.length; i++) {
        ...
        m.x -= m.sp; m.y += m.sp;
        const tailX = m.x + m.len * 0.7, tailY = m.y - m.len * 0.7;
        ...
        ctx.shadowColor = `rgba(${accent[0]},${accent[1]},${accent[2]},0.7)`; ctx.shadowBlur = 8;
  ...
  return <canvas ref={canvasRef} className="arc-canvas" />;
}
